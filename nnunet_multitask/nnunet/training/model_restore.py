...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    try:
        trainer.process_plans(info['planses'][0])
    except:
        try:
            trainer.process_plans(info['plans'][0]) # For legacy multitasking implementation
        except:
            trainer.process_plans(info['plans']) # For legacy multitasking implementation
...
def load_model_and_checkpoint_files(folder, folds=None, mixed_precision=None, checkpoint_name="model_best"):
    ...
    trainer.output_folders = [folder] # TODO: 
    trainer.output_folder_bases = [folder]
