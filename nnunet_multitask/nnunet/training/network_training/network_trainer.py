...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.networks: list[Tuple[SegmentationNetwork, nn.DataParallel]] = None
        self.optimizers = None
        self.lr_scheduler = None
        self.tr_gens = self.val_gens = None
        ...
        self.datasets = None  # these can be None for inference mode
        self.dataset_trs = self.dataset_vals = None  # do not need to be used, they just appear if you are using the suggested load_dataset_and_do_split
        ...
        if self.log_files is None:
            ...
            for output_folder in self.output_folders:
                self.log_files.append(
                    join(output_folder, "training_log_%d_%d_%d_%02.0d_%02.0d_%02.0d.txt" % (timestamp.year, timestamp.month, timestamp.day, timestamp.hour, timestamp.minute, timestamp.second))
                )
    ...
    def save_checkpoint(self, fname, save_optimizer=True, network=None, optimizer=None):
        start_time = time()
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        self.current_network_id = 0 # For only 0-th network inference
        curr_state_dict_keys = list(self.networks[self.current_network_id].state_dict().keys()) # For only a single network inference
        ...
        self.networks[self.current_network_id].load_state_dict(new_state_dict) # For only a single network inference
        self.epoch = checkpoint['epoch']
        if train:
            optimizer_state_dict = checkpoint['optimizer_state_dict']
            if optimizer_state_dict is not None:
                self.optimizers[self.current_network_id].load_state_dict(optimizer_state_dict) # For only a single network inference
    ...
    def run_training(self):
        ...
        for tr_gen in self.tr_gens:
            _ = tr_gen.next()
        for val_gen in self.val_gens:
            _ = val_gen.next()
        ...
        while self.epoch < self.max_num_epochs:
            ...
            if self.use_progress_bar:
                with trange(self.num_batches_per_epoch) as tbar:
                    for b in tbar:
                        ...
                        l = self.run_iteration(self.tr_gens, True)
            ...
            with torch.no_grad():
                ...
                for b in range(self.num_val_batches_per_epoch):
                    l = self.run_iteration(self.val_gens, False, True)
                    val_losses.append(l)
        ...
        if self.save_final_checkpoint:
            for output_folder, network, optimizer in zip(self.output_folders, self.networks, self.optimizers):
                self.save_checkpoint(join(output_folder, "model_final_checkpoint.model"), network=network, optimizer=optimizer)
        # now we can delete latest as it will be identical with final
        for output_folder in self.output_folders:
            if isfile(join(output_folder, "model_latest.model")): os.remove(join(output_folder, "model_latest.model"))
            if isfile(join(output_folder, "model_latest.model.pkl")): os.remove(join(output_folder, "model_latest.model.pkl"))
    ...
    def maybe_save_checkpoint(self):
        ...
        if self.save_intermediate_checkpoints and (self.epoch % self.save_every == (self.save_every - 1)):
            ...
            if not self.save_latest_only:
                for output_folder, network, optimizer in zip(self.output_folders, self.networks, self.optimizers):
                    self.save_checkpoint(join(output_folder, "model_ep_%03.0d.model" % (self.epoch + 1)), network=network, optimizer=optimizer)
            for output_folder, network, optimizer in zip(self.output_folders, self.networks, self.optimizers):
                self.save_checkpoint(join(output_folder, "model_latest.model"), network=network, optimizer=optimizer)
        ...
        if self.patience is not None:
            ...
            if self.val_eval_criterion_MA > self.best_val_eval_criterion_MA:
                ...
                if self.save_best_checkpoint:
                    for output_folder, network, optimizer in zip(self.output_folders, self.networks, self.optimizers):
                        self.save_checkpoint(join(output_folder, "model_best.model"), network=network, optimizer=optimizer)
            ...
            if self.epoch - self.best_epoch_based_on_MA_tr_loss > self.patience:
                if self.optimizers[0].param_groups[0]['lr'] > self.lr_threshold:
