    ...
    def resample_and_normalize(self, data, target_spacing, properties, seg=None, force_separate_z=None):
        ...
        for c in range(len(data)):
            ...
            if scheme == "CT":
                ...
                # uni, cnt = np.unique(data[c], return_counts=True)
                # for i in range(len(cnt)-1, -1, -1):
                #     if cnt[i] >= int(cnt.mean() * 0.01):
                #         target_idx = i
                #         break
                # window_width = uni[target_idx]
                # window_level = window_width / 2 # window_center
                # data[c] = np.clip(data[c], window_level - (window_width / 2), window_level + (window_width / 2)) # Windowing

                # Basic Normalization
                # mn = data[c].mean()
                # std = data[c].std() + 1e-8
                # data[c] = (data[c] - mn) / std

                # Nonzero-mask Normalization
                # mask = seg[-1] >= 0 # mask에서 -1을 제외한 0 이상의 값의 위치
                # mn = data[c][mask].mean()
                # std = data[c][mask].std() + 1e-8
                # data[c][mask] = (data[c][mask] - mn) / std
                # data[c][mask == False] = 0 # mask==0 -> -1인 영역을 0으로 취함

                # MONAI Normalization
                # slices = data[c] != 0
                # mn = data[c].mean()
                # std = data[c].std() + 1e-8
                # data[c][slices] = (data[c][slices] - mn) / std
                
                # Distribution-max Normalization
                # mask = seg[-1] >= 0 # mask에서 Background(-1)을 제외한 Foreground(0 이상의 값)의 위치
                # mn = data[c][mask].mean()
                # std = data[c][mask].std() + 1e-8
                # data[c][mask] = (data[c][mask] - mn) / std # Normalization
                # uni, cnt = np.unique(data[c][mask], return_counts=True) # Foreground 영역의 고유값
                # data[c][mask == False] = uni[np.argmax(cnt)] # 고유값 중 가장 빈도가 높은 값을 Background에 할당
