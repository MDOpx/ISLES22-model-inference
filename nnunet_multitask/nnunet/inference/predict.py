...
def preprocess_multithreaded(trainer, list_of_lists, output_files, num_processes=2, segs_from_prev_stage=None):
    ...
    for i in range(num_processes):
        pr = Process(target=preprocess_save_to_queue, args=(trainer.preprocess_patient, q,
                                                            ...
                                                            classes, trainer.planses[0]['transpose_forward']))
        pr.start()
        processes.append(pr)
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    if segmentation_export_kwargs is None:
        if 'segmentation_export_params' in trainer.planses[0].keys(): # TODO: [0]으로 바꿔도 되려나?
            force_separate_z = trainer.planses[0]['segmentation_export_params']['force_separate_z']
            interpolation_order = trainer.planses[0]['segmentation_export_params']['interpolation_order']
            interpolation_order_z = trainer.planses[0]['segmentation_export_params']['interpolation_order_z']
        else:
    ...
    for preprocessed in preprocessing:
        ...
        transpose_forward = trainer.planses[0].get('transpose_forward')
        if transpose_forward is not None:
            transpose_backward = trainer.planses[0].get('transpose_backward')
...
def predict_cases_fast(model, list_of_lists, output_filenames, folds, num_threads_preprocessing,
    ...
    if segmentation_export_kwargs is None:
        if 'segmentation_export_params' in trainer.planses[0].keys():
            force_separate_z = trainer.planses[0]['segmentation_export_params']['force_separate_z']
            interpolation_order = trainer.planses[0]['segmentation_export_params']['interpolation_order']
            interpolation_order_z = trainer.planses[0]['segmentation_export_params']['interpolation_order_z']
        else:
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
                        ...
                        segmentation_export_kwargs: dict = None, disable_postprocessing: bool = False, pkl_folder: str = None):
    """
