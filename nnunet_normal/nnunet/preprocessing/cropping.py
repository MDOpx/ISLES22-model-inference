...
def crop_to_nonzero(data, seg=None, nonzero_label=-1,pkl_file=None):
    """
    ...
    if pkl_file == None:
        ...
        with open(pkl_file, 'rb') as f:
            properties_tmp = pickle.load(f)
            bbox = properties_tmp.get('crop_bbox')
    cropped_data = []
    for c in range(data.shape[0]):
...
class ImageCropper(object):
    ...
    @staticmethod
    def crop(data, properties, seg=None, pkl_file = None):
        shape_before = data.shape
        data, seg, bbox = crop_to_nonzero(data, seg, nonzero_label=-1, pkl_file=pkl_file)
    ...
    @staticmethod
    def crop_from_list_of_files(data_files, seg_file=None,pkl_folder=None):
        data, seg, properties = load_case_from_list_of_files(data_files, seg_file)
        if pkl_folder != None:
            pkl_name = data_files[0].split('/')[-1][:-12]
            pkl_file=f'{pkl_folder}/{pkl_name}.pkl'
            print(f'pkl file name : {pkl_file}')
        else:
            pkl_file = None
        
        return ImageCropper.crop(data, properties, seg,pkl_file)
